...
@section('content')
    ...
    <div class="min-h-screen flex">
        ...
        <div class="hidden lg:flex flex-col justify-between w-[45%] relative overflow-hidden p-14 left-panel">
            <div class="absolute w-[400px] h-[400px] rounded-full blur-3xl opacity-20 -top-20 -left-20"
                style="background:#4ade80"></div>
            <div class="absolute w-80 h-80 rounded-full blur-3xl opacity-15 bottom-20 right-10" style="background:#a855f7">
            </div>
                ...
                <div class="space-y-5">
                    @foreach([
                        ...
                        <div class="flex items-start gap-4">
                            <div
                                class="w-6 h-6 rounded-full bg-green-400/20 border border-green-400/40 flex items-center justify-center flex-shrink-0 mt-0.5">
            ...
            <p class="text-white/20 text-xs font-semibold relative z-10">&copy; {{ date('Y') }} Edfrica. COPPA &amp; GDPR-K
                Compliant.</p>
        </div>
        ...
        <div class="flex-1 flex items-center justify-center p-6 sm:p-12 bg-white overflow-y-auto">
            <div class="w-full max-w-md py-8 register-container form-fade">
                ...
                @if($errors->any())
                    <div class="flash flash-error">@foreach($errors->all() as $e)
                    <p>{{ $e }}</p>@endforeach
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('register') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input id="reg_pass" type="password" name="password" required class="form-input"
                            placeholder="At least 8 characters">
                    </div>
                    <div>
                        <label for="reg_pass2" class="form-label">Confirm Password</label>
                        <input id="reg_pass2" type="password" name="password_confirmation" required class="form-input"
                    ...
                    <div class="flex items-start gap-3 pt-1">
                        ...
                        <label for="terms" class="text-xs font-semibold text-muted leading-relaxed">
                            I agree to TLab's <a href="#" class="text-primary font-black hover:underline">Terms of
                                Service</a> and <a href="#" class="text-primary font-black hover:underline">Privacy
                                Policy</a>.
                        </label>
                    </div>
