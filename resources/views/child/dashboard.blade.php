...
@php
    $rankColors = [
        'Explorer'       => ['c'=>'#4DFFA2','lv'=>1],
        'Innovator'      => ['c'=>'#5AD7FF','lv'=>2],
        'Builder'        => ['c'=>'#FFD93D','lv'=>3],
        'Creator'        => ['c'=>'#FF6BB5','lv'=>4],
        'Master Inventor'=> ['c'=>'#9B7BFF','lv'=>5],
    ];
    $rc = $rankColors[$child->rank] ?? $rankColors['Explorer'];
    $first = explode(' ', $child->name)[0];
    $planetPath = [
        ['Explorer', 0, 'E', '#4DFFA2'],
        ['Innovator', 200, 'I', '#5AD7FF'],
        ['Builder', 500, 'B', '#FFD93D'],
        ['Creator', 1000, 'C', '#FF6BB5'],
        ['Master Inventor', 2000, 'M', '#9B7BFF'],
    ];
    $missionColors = [
        'stem-club'      => ['c'=>'#4DFFA2','gl'=>'S'],
        'brain-club'     => ['c'=>'#5AD7FF','gl'=>'B'],
        'art-craft-club' => ['c'=>'#FF6B4D','gl'=>'A'],
        'leadership-club'=> ['c'=>'#9B7BFF','gl'=>'L'],
    ];
@endphp
...
<section class="relative overflow-hidden rounded-[2rem] border-[3px] border-cream/25 hard-shadow mb-8 reveal pop-in"
    ...
    <div class="absolute -right-10 -top-10 opacity-25"><span class="geo-ring" style="width:110px;height:110px"></span></div>
    <div class="absolute -left-8 -bottom-12 opacity-20"><span class="geo-diamond" style="width:72px;height:72px"></span></div>
    ...
    <div class="relative z-10 p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center gap-5">
        <div class="relative flex-shrink-0">
            ...
            <div class="absolute -top-2 -right-2 w-9 h-9 rounded-full grid place-items-center font-black text-base bg-cream border-[2.5px] border-space shadow-[3px_3px_0_#0a0718] bounce-in">
                {{ $rc['lv'] }}
            </div>
        </div>
        ...
        <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 flex-wrap mb-1">
                ...
                <span class="sticker bg-space text-cream px-3 py-1 text-xs font-black">
                    <span class="inline-block w-2 h-2 rounded-full mr-1.5 align-middle" style="background:{{ $rc['c'] }}"></span>{{ $child->rank }}
                </span>
            </div>
            ...
            <div class="mt-4 max-w-md">
                ...
                <div class="h-4 rounded-full bg-space/20 border-2 border-space overflow-hidden relative">
                    <div class="h-full rounded-full relative transition-all duration-1000 ease-out" id="xp-bar" data-progress="{{ $child->rank_progress }}"
                        ...
                        <span class="absolute -right-2 top-1/2 -translate-y-1/2 w-3.5 h-3.5 bg-space rotate-45 rounded-[3px] border-2 border-cream/90"></span>
                    </div>
                </div>
            </div>
        </div>
...
<section class="candy-card hard-shadow p-5 sm:p-6 mb-8 reveal" data-delay="60">
    <div class="flex items-center justify-between flex-wrap gap-2 mb-5">
        <h2 class="font-display font-extrabold text-lg text-cream flex items-center gap-2">
            <span class="inline-block"><span class="geo-ring"></span></span> YOUR PLANET PATH
        </h2>
    ...
    <div class="flex items-start justify-between gap-1 sm:gap-2 overflow-x-auto pb-2">
        @foreach($planetPath as [$rankName, $threshold, $glyph, $color])
            @php
            ...
            <div class="flex flex-col items-center flex-shrink-0 px-1" style="min-width:74px">
                <div class="planet-core {{ (!$achieved && !$isCurrent) ? 'planet-locked' : '' }} relative w-16 h-16 sm:w-18 sm:h-18 rounded-full grid place-items-center font-black text-2xl sm:text-3xl border-[3px] border-space mb-2"
                    ...
                    @elseif($achieved)
                        <span class="absolute -top-1 -right-1 w-5 h-5 rounded-full grid place-items-center text-[10px] font-black text-space bg-mint border-2 border-space shadow-[2px_2px_0_#0a0718]">✓</span>
                    @endif
                </div>
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <section class="candy-card hard-shadow p-5 sm:p-6 reveal" data-delay="100">
            <div class="flex items-center justify-between mb-5">
                <h2 class="font-display font-extrabold text-lg text-cream flex items-center gap-2">
                    <span class="inline-block"><span class="geo-plus"></span></span> MY MISSIONS
                </h2>
            ...
            @if($child->enrollments->isEmpty())
                <div class="text-center py-12">
                    <div class="text-6xl mb-6 float inline-flex"><span class="geo-outbox"></span></div>
            ...
            @else
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    @php
                        $cc = $missionColors[$enrollment->course->club->slug ?? ''] ?? ['c'=>'#FFD93D','gl'=>'C'];
                       ...
                       class="group relative rounded-[1.4rem] bg-cream border-[3px] border-space p-5 hard-shadow-sm transition-all duration-250 hover:-translate-y-1 hover:rotate-[0.6deg] hover:shadow-[8px_8px_0_#0a0718] active:scale-[0.98] block">
                        <div class="flex items-center gap-3 mb-4">
                            <div class="w-12 h-12 rounded-2xl grid place-items-center text-2xl border-[2.5px] border-space flex-shrink-0 shadow-[3px_3px_0_#0a0718]"
                                ...
                                <span class="font-black">{{ $cc['gl'] }}</span>
                            </div>
                            <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center gap-3 mb-4">
                            <div class="flex-1 h-3 rounded-full border-2 border-space bg-white overflow-hidden">
                                <div class="h-full transition-all duration-700 relative" style="width:{{ $courseProgress }}%;background:{{ $cc['c'] }}">
                                    <span class="absolute -right-2 top-1/2 -translate-y-1/2 w-3 h-3 bg-space rotate-45 rounded-[2px] border-2 border-white"></span>
                                </div>
                            </div>
        ...
        <section class="candy-card hard-shadow p-5 sm:p-6 reveal" data-delay="140">
            <h2 class="font-display font-extrabold text-lg text-cream flex items-center gap-2 mb-5">
                <span class="inline-block"><span class="geo-diamond"></span></span> BIG XP MOMENTS
            </h2>
            @if($child->xpLogs->isEmpty())
                <div class="text-center py-10">
                    <div class="mb-4 float inline-flex"><span class="geo-diamond" style="width:44px;height:44px;opacity:.85"></span></div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between gap-3 rounded-2xl border-2 border-cream/10 bg-surface/60 px-4 py-3 hover:border-cream/25 transition-colors">
                        <div class="flex items-center gap-3 min-w-0">
                            <div class="w-10 h-10 rounded-xl grid place-items-center text-lg border-2 border-space shadow-[2px_2px_0_#0a0718]" style="background:rgba(255,217,61,.9)"><span class="font-black text-space">+</span></div>
                            <div class="min-w-0">
    ...
    <div class="space-y-6">
        <section class="candy-card hard-shadow p-5 sm:p-6 reveal" data-delay="180">
            <h2 class="font-display font-extrabold text-lg text-cream flex items-center gap-2 mb-1">
                <span class="inline-block"><span class="geo-ring" style="width:26px;height:26px"></span></span> TOP PILOTS
            </h2>
            ...
            <div class="space-y-2.5">
                ...
                @php
                    ...
                    $podium = $i === 0 ? ['#FFD93D','1'] : ($i === 1 ? ['#E8F0F8','2'] : ($i === 2 ? ['#FFB347','3'] : null));
                @endphp
                <div class="flex items-center gap-3 rounded-2xl border-2 px-3 py-2.5 transition-all {{ $isYou ? 'border-mint bg-mint/10' : 'border-cream/10 bg-surface/50' }}">
                    <span class="w-7 flex justify-center">
                        @if($podium)
                            <span class="inline-flex items-center justify-center w-7 h-7 rounded-full border-2 border-space text-[11px] font-black text-space shadow-[2px_2px_0_#0a0718]" style="background:{{ $podium[0] }}">{{ $podium[1] }}</span>
                        @else
                            <span class="text-cream/30 text-xs">#{{ $i + 1 }}</span>
                        @endif
                    </span>
        ...
        <section class="relative overflow-hidden candy-card hard-shadow p-5 sm:p-6 reveal" data-delay="220"
            ...
            <div class="absolute -right-6 -bottom-8 opacity-20 float"><span class="geo-plus" style="width:56px;height:56px"></span></div>
            <div class="museum text-sky mb-2 blink-caret">// DID YOU KNOW //</div>
            <p class="text-cream/80 font-extrabold text-sm leading-relaxed">
                Completing quizzes fires at <span class="text-gold">+XP</span> · finishing a module boosts your rank progress
