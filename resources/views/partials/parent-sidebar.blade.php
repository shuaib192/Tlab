@php
    ...
    $unreadCount = auth()->check() ? Notification::where('user_id', auth()->id())->where('is_read', false)->count() : 0;
    $navItems = [
        ...
        'notifications.index' => ['Notifications', 'M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9'],
        'parent.subscription' => ['Subscription', 'M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z'],
    ];
@endphp
    ...
    <nav class="p-4 space-y-1 overflow-y-auto" style="height:calc(100% - 140px)">
        ...
        <a href="{{ route($route) }}"
           ...
           onmouseout="this.style.background='{{ $active ? 'rgba(22,163,74,0.12)' : 'transparent' }}'; this.style.color='{{ $active ? '#16A34A' : 'rgba(255,255,255,0.5)' }}'">
            ...
            {{ $label }}
            @if($route === 'notifications.index' && $unreadCount > 0)
            <span class="ml-auto text-[10px] font-bold bg-primary text-white px-2 py-0.5 rounded-full min-w-[20px] text-center">{{ $unreadCount > 99 ? '99+' : $unreadCount }}</span>
            @endif
        </a>
        @endforeach
    </nav>
